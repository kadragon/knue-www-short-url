 ...
 *
 * @param {Object} params - 인코딩할 파라미터 객체
 * @param {string} params.site - 사이트명 (knueSites.js의 siteMap에 정의된 값)
 * @param {number} params.key - 게시물 키 (유효한 숫자여야 함, validators.js 참조)
 * @param {number} params.bbsNo - 게시판 번호 (유효한 숫자여야 함)
 * @param {number} params.nttNo - 게시글 번호 (유효한 숫자여야 함)
 * @returns {Object} 인코딩 결과
 * @returns {string} result.code - 성공 시 단축 코드 (예: "abc123")
 * @returns {string} result.error - 실패 시 에러 메시지
 *
 * @example
 * // 성공 사례
 ...
 * @example
 * // 실패 사례 - 유효하지 않은 숫자
 * const result = encodeURL({ site: "www", key: null, bbsNo: 456, nttNo: 789 });
 * // result: { error: "key, bbsNo, nttNo는 반드시 숫자여야 합니다." }
 */

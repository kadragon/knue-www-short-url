...
- [ ] `test/` 타입 안전성 — `tsconfig.json` `include`가 `["src"]`라 테스트는 tsc 검사 대상 밖이고 `@types/node`도 미설치. `test/seo.test.ts`의 `node:fs`/`__dirname`이 런타임에만 동작. `@types/node` 추가 + 테스트 전용 tsconfig 또는 lint 설정으로 가드. Effort S.

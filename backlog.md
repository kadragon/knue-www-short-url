...
- [ ] [TOOLING] `test/` 타입 안전성 — `tsconfig.json` `include`가 `["src"]`라 테스트는 tsc 검사 대상 밖이고 `@types/node`도 미설치. `test/seo.test.ts`의 `node:fs`/`node:path`/`process`가 런타임에만 동작하고 에디터에는 오류로 보임. `@types/node` 추가 + 테스트 포함 tsconfig(또는 별도 typecheck 스크립트)로 가드. Effort S.

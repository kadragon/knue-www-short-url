...
- 의존성: qrcode.js 라이브러리 (동적 `await import('qrcode')`로 지연 로딩, 별도 청크로 code-split)
...
- **AC-3**: 생성 실패 시 중앙 구조화 로거로만 기록 (사용자 알림 없음)
  ```gherkin
  GIVEN QRCode.toCanvas() 실패
  WHEN 콜백의 error 전달됨
  THEN logError('QRCode', error) 호출 (console 기반, 사용자 알림 없음)
  ```
...
**Library**: qrcode.js (`npm install qrcode`), 지연 로딩

```javascript
const { default: QRCode } = await import('qrcode'); // lazy, separate chunk
...
**Error Handling**:
- error ≠ null: logError('QRCode', error) 기록 (console 기반), 사용자 알림 미제공
